export const generateExplanation = (regex: RegExp, regexFlags: string) => {
  ...
  if (source.includes("\\w")) explanation += "\\w - Matches any letter, number, or underscore (_)\n";
  ...
  if (source.includes("{")) explanation += "{n} - Matches exactly 'n' times (like {3} means 3 times)\n";
  if (source.includes("|")) explanation += "| - Means 'or' (matches this or that)\n";
  if (source.includes("[") && source.includes("]")) explanation += "[] - Matches any one of the characters inside the brackets\n";
  if (source.includes("(") && source.includes(")")) explanation += "() - Groups things together\n";
  if (source.includes("\\b")) explanation += "\\b - Matches the edge of a word (like a word boundary)\n";

  // Flags explanation
  if (regexFlags.includes("i")) explanation += "i flag - Ignores uppercase and lowercase differences\n";
  if (regexFlags.includes("g")) explanation += "g flag - Finds all matches, not just the first one\n";
  if (regexFlags.includes("m")) explanation += "m flag - Lets ^ and $ match the start/end of each line (not just the whole string)\n";
