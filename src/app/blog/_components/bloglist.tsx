...
export default function BlogList({ posts }: { posts: ContentMetadata[] }) {
  return (
    <ul className=" flex flex-col gap-4 mt-6 w-full">
      {posts.map((post) => (
        <li key={post.slug} className=" border-dashed border-b border-stone-300 dark:border-stone-700 group">
          <Link href={`/blog/${post.slug}`} className="transition-all duration-300 flex items-center justify-between py-2">
            <Icon
            ...
            />
            <span className="flex-1 line-clamp-1 transition-all duration-300 group-hover:translate-x-1 group-hover:dark:text-stone-200">{post.title}</span>
